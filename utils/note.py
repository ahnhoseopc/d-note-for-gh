...
#
# Operation Report (수술기록지) Data Sources
#
def collect_or_source(patient_id, admsn_date):
    ...
    or_source = {
        ...
        "obsteric gpal": None if len(df_ae) > 0 else df_ay["ocm41ohg"][0]+'/'+df_ay["ocm41ohp"][0]+'/'+df_ay["ocm41oha"][0]+'/'+df_ay["ocm41ohl"][0] if len(df_ay) > 0 else None,
        "menstrual history": None if len(df_ae) > 0 else df_ay["ocm41lmp"][0]+'/'+df_ay["ocm41pmp"][0]+'/'+df_ay["ocm41interval"][0]+'/'+df_ay["ocm41menache"][0] if len(df_ay) > 0 else None,
        ...
        "family history": df_ae["ocm31family"][0] if len(df_ae) > 0 else df_ay["ocm41family"][0] if len(df_ay) > 0 else None,

        "review of systems": None if len(df_ae) > 0 else df_ay["ocm41ros"][0] if len(df_ay) > 0 else None,
        "other review of systems": df_ay["ocm31rosother"][0] if len(df_ae) > 0 else df_ay["ocm41rosother"][0] if len(df_ay) > 0 else None,

        "impression": df_ae["ocm31imp"][0] if len(df_ae) > 0 else df_ay["ocm41imp"][0] if len(df_ay) > 0 else None,
        "plan": df_ae["ocm31plan"][0] if len(df_ae) > 0 else df_ay["ocm41plan"][0] if len(df_ay) > 0 else None,
        ...
        "report time": df_ae["ocm31systm"][0] if len(df_ae) > 0 else df_ay["ocm41systm"][0] if len(df_ay) > 0 else None,
    }
    ...
    or_current = {
        ...
        "doctor assistant": df_or["ocm06rgcd"][0] if len(df_or) > 0 else None,
        "comment": df_or["ocm06cmta"][0] if len(df_or) > 0 else None,
        "operation procedures": df_or["ocm06cmtb"][0] if len(df_or) > 0 else None,
...
#
# Discharge Summary Report (퇴원요약지) Data Sources
#
